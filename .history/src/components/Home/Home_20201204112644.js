...
const Home = () => {
  ...
  console.log(
    question == null
      ? null
      : (C = Object.keys(question).filter((key) =>
          H.forEach((i) => {
            question[key].id !== i
              ? console.log(question[key].id)
              : (index = Object.keys(question).indexOf(question[key].id));
          })
          index > -1 ? Object.keys(question).slice(index, 1) : false;
        ))
  );
